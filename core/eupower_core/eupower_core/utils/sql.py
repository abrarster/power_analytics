...
__all__ = ["prepare_query", "prepare_query_from_string"]
...
class JinjaSql:
    ...
    def _prepare_environment(self):
        ...
        if int(jinja2.__version__.split(".")[0]) < 3:
...
def prepare_query_from_string(query_template: str, params: dict[str, str]) -> str:
    j = JinjaSql(param_style="pyformat")
    query, bind_params = j.prepare_query(query_template, params)
    prepared_query = get_sql_from_template(query, bind_params) % bind_params
    return prepared_query


def prepare_query(query_template_path: str, params: dict[str, str]) -> str:
    with open(query_template_path, "r") as f:
        query_template = f.read()
    return prepare_query_from_string(query_template, params)
